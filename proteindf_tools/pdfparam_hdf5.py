...
class PdfParam_H5(PdfParamObject):
    ...
    def _write_sp_molecule(self, h5grp):
        ...
        h5ds_molecule.attrs["header"] = ["symbol", "label", "x", "y", "z", "charge"]
    ...
    def _write_sp_basisset(self, h5grp):
        ...
        for atom_label in self.get_basisset_atomlabels():
            ...
            for cgto_id, cgto in enumerate(basisset):
                ...
                for pgto_id, pgto in enumerate(cgto):
                    coef = pgto.coef
                    exp = pgto.exp
                    h5_pgto.append([name, int(cgto_id), int(pgto_id), coef, exp])
        ...
        h5ds_cgto.attrs["header"] = ["name", "cgto_id", "shell_type", "scale_factor"]

        h5ds_pgto = h5grp.create_dataset("basisset/pgto", data=h5_pgto)
        h5ds_pgto.attrs["header"] = ["name", "cgto_id", "pgto_id", "coef", "exp"]


    def _write_sp_condition_control(self, h5grp):
        h5grp_control = h5grp.create_group("control")
        h5grp_control_file_base_name = h5grp_control.create_group("file_base_name")

        control_file_base_name = self._data["control"].get("file_base_name", {})
