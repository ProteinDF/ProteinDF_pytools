...
class Ridge(object):
    ...
    def fit_lagrangian(self, X, y):
        ...
        MinvX = Minv * tX
        #MinvX = pdf.Matrix(MinvX)
        #MinvX.save("M_linv.mat")
        self._coef = MinvX * y
...
def main():
    ...
    if use_scikit_learn:
        ...
        #ridge.fit(X, y)
        ridge.fit_lagrangian(X, y)
        atom_charges0 = ridge.coef[:-1]
        ridge.fit_atomic_charges(X, y)
        atom_charges = ridge.coef[:-1]

    # set atomic charge to atom list (0)
    atomlist0 = copy.deepcopy(atomlist)
    q_total = 0.0
    for i in range(len(atomlist)):
        q = atom_charges0[i];
        atomlist0[i].charge = q;
        q_total += q
        print(atomlist0[i])
    print('charge0={:.3f}'.format(q_total))
